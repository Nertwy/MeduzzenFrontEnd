<template>
    ...
    <template v-if="pageData">
        <Basic_Table :keys="keys" :data="pageData.results" class='table table-zebra' :edit="null">
            <template #th-slot>
                <th>
            ...
            <template #td-slot="{ id, index }">
                <td v-if="Requests?.find((requests) => requests.company === id)">
                    <Basic_button button-text="Cancel Request" class="btn btn-error" @click-func="handleCancel(id)" />
                </td>
                <td v-else>
                    <Basic_button button-text="Request to join" class="btn btn-success" @click-func="handleClick(id)" />
                </td>
            </template>
        </Basic_Table>
...
const handleCancel = async (id?: number) => {
    if (!id) return
    const result = await postReqAxios(`api/companies/${id}/cancel_join_request/`, {})
    ...
    Requests.value = Requests.value?.filter(item => item.company !== id) ?? null
    triggerToast(result)
}
const fetch = async () => {
    ...
    if (result.results[0])
        keys.value = Object.keys(result.results[0])
    const token = localStorage.getItem("access")
    const pendingInvites = await getReqAxios("api/companies/get_pending_join_requests/", {
        headers: {
