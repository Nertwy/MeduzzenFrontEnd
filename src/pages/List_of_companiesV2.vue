<template>
    <section>
        ...
        <template v-else>
            <Basic_Table :keys="keys" :data="pageData.results" class='table table-zebra' :td-layout="Edit_Input"
                v-model="handleDataRef" :edit="edit">
                <template #th-slot>
                    <th>
                        Edit
                    </th>
                ...
                <template #td-slot="{ id, index, value }">
                    <td>
                        <Edit_Button button-text="Edit" @edit-click="() => handleEditClick(index)"
                            @edit-cancel="() => handleEditClick(null)" :edit="edit === index"
                            :edit-function-submit="() => { }" />
                    </td>
                    <td>
                        <ModalWindow :btn-open-text="'Delete'" :title="'Delete company?'" :text="'All data will be lost!'">
                            <div class="flex flex-col gap-2">
                                <label for="confirm_pass_input">Write your password to confirm deletion</label>
                                <BaseInput class="input input-accent" v-model="userPassword" />
                                <Delete_Button :delete-function="() => deleteFunc?.(id ?? -1, userPassword)" />
                            </div>
